...
This style is superseded. The Warm Editorial Journal export of 2026-09-23 replaced it, and that one is in `docs/design/prototype/` with its document in `docs/design/prototype-design-system.md`. Read `docs/design/warm-humanist-editorial/README.md` for why this one is still here and which two steps delete it.

The prototype this document describes is in `docs/design/warm-humanist-editorial/`. The front matter is read out of `today-dashboard.html`, which carries the configuration all five screens draw with, and it is not written by hand. `packages/tokens/tests/designSystem.test.ts` holds the two together.
