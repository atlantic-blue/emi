...
114 exported symbols, across 8 files.
...
```
interface SealProfileOptions extends ProfileOptions {
  /** Where the nonce comes from. Left out, it is the platform's own generator. */
  readonly random?: RandomSource;
}
```
...
```
function sealProfile(profile: ProfileRecord, key: Uint8Array, options: SealProfileOptions = {}): Uint8Array
```
...
```
function openProfile(envelope: Uint8Array, key: Uint8Array): ProfileRecord
```
...
```
function openPulledRecord(envelope: Uint8Array, key: Uint8Array): PulledRecord
```
...
## `packages/crypto/src/profile.ts`
...
```
type Regularity = 'regular' | 'moves' | 'unknown';
```
...
```
type Feeling = 'fine' | 'hard' | 'understand';
```
...
```
type Goal = 'forecast' | 'symptoms' | 'fertileWindow' | 'doctorRecord';
```
...
```
const regularityValues = ['regular', 'moves', 'unknown'] as const satisfies readonly Regularity[]
```
...
```
const feelingValues = ['fine', 'hard', 'understand'] as const satisfies readonly Feeling[]
```
...
```
const goalValues = ['forecast', 'symptoms', 'fertileWindow', 'doctorRecord'] as const satisfies readonly Goal[]
```
...
```
const focusValues = ['sleep', 'mood', 'energy', 'skin', 'digestion', 'pain'] as const satisfies readonly Focus[]
```
...
```
const profileKeys: readonly string[]
```
...
```
const shortestCycleLengthDays = 21
```
...
```
const longestCycleLengthDays = 45
```
...
```
const shortestPeriodLengthDays = 1
```
...
```
const longestPeriodLengthDays = 15
```
...
```
function profileProblems(profile: ProfileRecord, options: ProfileOptions = {}): readonly string[]
```
...
```
function checkedProfile(profile: ProfileRecord, options: ProfileOptions = {}): ProfileRecord
```
...
```
function profileJson(profile: ProfileRecord, options: ProfileOptions = {}): string
```
...
```
function profileBytes(profile: ProfileRecord, options: ProfileOptions = {}): Uint8Array
```
...
```
function profileFromBytes(bytes: Uint8Array): ProfileRecord
```
...
```
function pulledFromBytes(bytes: Uint8Array): PulledRecord
```
