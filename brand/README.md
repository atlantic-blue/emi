...
`icons/` holds the twenty eight line symbols and the contact sheet. `icons/README.md` says the rules
...
Every picture goes through `screens/picture.ts`, which commits the markup it drew beside the image,
so `npm run check:pictures` renders the screens again and compares the markup without a browser. The
comparison is on the markup and never on the image, because two browsers draw one page into
different bytes. That check counts the picture files on disk and fails when it compared fewer than
it found, so a picture added with no comparison stops the run rather than sitting there unread. `docs/story.md` shows these pictures to a reader.
