...
99 exported symbols, across 8 files.
...
```
const DAYS_EITHER_SIDE_WHILE_LEARNING = Math.round(POPULATION_SPREAD_DAYS)
```
...
```
function startWhileLearning(lastStartedOn: string, statedCycleLengthDays: number): DayRange
```
...
## `packages/cycle/src/patterns.ts`

### `DayOfSymptoms`

```
interface DayOfSymptoms {
  readonly day: string;
...
```
const PATTERN_WINDOW_CYCLES = FORECAST_WINDOW_CYCLES
```
...
### `PATTERN_DAY_TOLERANCE`

```
const PATTERN_DAY_TOLERANCE = 1
```
...
```
type PatternAnchor = 'days-before-the-period' | 'cycle-day';
```
...
```
function slugsOn(record: DayOfSymptoms): string[]
```
...
```
function windowsOf(cycles: readonly Cycle[], windowCycles: number): Window[]
```
...
```
function patternsIn(from: PatternsFrom): SymptomPattern[]
```
