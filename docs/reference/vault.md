...
84 exported symbols, across 14 files.
...
## `services/vault/src/handlers/deleteAccount.ts`

### `accountPath`

```
const accountPath = '/v1/account'
```
...
```
function deleteAccountFor(event: HttpRequestEvent, store: AccountDeleteStore): Promise<HttpResponse>
```
...
```
function deleteAccount(store: AccountDeleteStore): (event: HttpRequestEvent) => Promise<HttpResponse>
```
...
## `services/vault/src/handlers/readArticles.ts`

### `phaseParameter`

```
const phaseParameter = 'phase'
```
...
```
const identityHeaders: readonly string[]
```
...
```
function identityHeadersIn(event: HttpRequestEvent): string[]
```
...
```
function authorizerRan(event: HttpRequestEvent): boolean
```
...
```
function phaseIn(event: HttpRequestEvent): PhaseName | null
```
...
```
function drawable(article: StoredArticle): boolean
```
...
```
function newestOf(articles: readonly StoredArticle[]): StoredArticle | null
```
...
```
function articlesFor(event: HttpRequestEvent, store: ArticleStore): Promise<HttpResponse>
```
...
```
function readArticles(store: ArticleStore): (event: HttpRequestEvent) => Promise<HttpResponse>
```
...
## `services/vault/src/store/articles.ts`
...
```
interface DeleteItemInput extends ExpressionParts {
  readonly TableName: string;
  readonly Key: Item;
  readonly ConditionExpression?: string;
}
...
```
function dynamoStore(table: DynamoDbTable, tableName: string): AccountStore & RecordStore & AccountDeleteStore
```
...
## `services/vault/src/store/dynamoArticles.ts`

### `articlePartitionFor`

```
function articlePartitionFor(phase: string): string
```
...
```
function articleSortKeyFor(slug: string): string
```
...
```
function dynamoArticleStore(table: DynamoDbTable, tableName: string): ArticleStore
```
